...
to setup-globals
  set infection-chance Infection
  set symptoms-show random-float 400
...
to assign-Infection
  set Infection random-near infection-chance
end
...
to go
  if all? turtles [infected? and known?]
    [ stop ]
  check-sliders
  ask turtles
      [if infected?
      ...
      if medicine-taken?
        [ set medicine-effect-length medicine-effect-length + 1 ] ]
  ask turtles
    [ if not coupled?
        [ move ] ]
  ask turtles
    [ if not coupled? and shape = "person righty" and (random-float 5.0 < coupling-tendency)
        [ couple ] ]
  ask turtles [ medicine-effect]
...
to check-sliders
  ...
  if (slider-check-3 != average-condom-use)
    [ ask turtles [ assign-condom-use ]
      set slider-check-3 average-condom-use ]
  if (infection-chance != Infection )
    [ ask turtles [ assign-Infection ]
      set Infection infection-chance ]
...
to medicine-effect
  if coupled? and not infected? and [infected? and known?] of partner
    [set medicine-taken? true]
  if medicine-effect-length > 28 + couple-length
    [set medicine-taken? false]
end
...
to couple
  let potential-partner one-of (turtles-at -1 0) with [not coupled? and shape = "person lefty"]
...
to infect
  if infected? and coupled?
   [if random-float 100 > condom-use or random-float 100 > ([condom-use] of partner)
      [ if known? and medicine-taken?
        [if random-float 400 < infection-chance
          [ ask partner [ set infected? true ]]]
      if not known? or (known? and not medicine-taken?)
      [ if random-float 75 < infection-chance
          [ ask partner [set infected? true ] ] ]]]
end
